"""
AM (Activation Matching) Test-Time Adaptation.

Implements ActMAD: multi-layer, location-aware activation alignment.

Usage::

    python adapt_am.py -c configs/tta/utkface_am.yaml -o result/tta_am/utkface
    python adapt_am.py -c configs/tta/svhn_am.yaml    -o result/tta_am/svhn
...
from tta.am_engine import AMEngine
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="AM (ActMAD) test-time adaptation.")
...
def run_single(config: dict[str, Any], args) -> dict[str, Any]:
    """Run AM adaptation + evaluation for a single configuration."""
    fix_seed(args.seed)
    ...
    # --- build AM engine ---------------------------------------------------
    am_cfg = config["tta"]["config"]
    engine = AMEngine(
        net=regressor,
        opt=opt,
        train_mode=am_cfg["train_mode"],
        act_stats_file=am_cfg["act_stats_file"],
        max_layers=am_cfg.get("max_layers", None),
        compile_model=am_cfg.get("compile_model", None),
    )
