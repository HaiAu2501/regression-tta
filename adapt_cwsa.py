"""
CWSA (Cramér–Wold Subspace Alignment) Test-Time Adaptation.

Usage::

    python adapt_cwsa.py -c configs/tta/utkface_cwsa.yaml -o result/tta_cwsa/utkface
...
from tta.cwsa_engine import CWSAEngine
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="CWSA-based test-time adaptation.")
...
def run_single(config: dict[str, Any], args) -> dict[str, Any]:
    """Run CWSA adaptation + evaluation for a single configuration."""
    fix_seed(args.seed)
    ...
    engine = CWSAEngine(regressor, opt, **config["tta"]["config"])
