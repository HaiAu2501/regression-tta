"""
RSD (Representation Subspace Distance) Test-Time Adaptation.
...
Usage::
    ...
    python adapt_rsd.py -c baselines/rsd/utkface.yaml -o result/tta_rsd/utkface
...
from tta.rsd_engine import RSDEngine
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="RSD-based test-time adaptation.")
...
def run_single(config: dict[str, Any], args) -> dict[str, Any]:
    """Run RSD adaptation + evaluation for a single configuration."""
    fix_seed(args.seed)
    ...
    engine = RSDEngine(regressor, opt, **config["tta"]["config"])
