"""
PSC (Predictive Spectral Calibration) Test-Time Adaptation.

Usage::

    python adapt_psc.py -c configs/tta/utkface_psc.yaml \
                        -o result/tta_psc/utkface
"""
...
from tta.psc_engine import PSCEngine
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="PSC test-time adaptation.")
...
def run_single(config: dict[str, Any], args) -> dict[str, Any]:
    """Run PSC adaptation + evaluation for a single configuration."""
    fix_seed(args.seed)
    ...
    engine = PSCEngine(regressor, opt, **config["tta"]["config"])
